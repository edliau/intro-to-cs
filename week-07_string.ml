...
(* Question 11 *)
let string_mapi_asaninstnaceof_string_init f s =
...
let test_check_string_map candidate =
  let b0 = (candidate (fun x -> x)  "" "" = true)
  and b1 = (candidate (fun x -> x) "abc" "acb" = false)
  and b2 = (candidate (fun a -> 'a') "123" "aaa" = true)
  and b3 = (candidate (fun b -> 'c') "abcd" "cccc" = true)
  and br = (candidate (fun c -> 'd') (String.make 5 (random_char())) "ddddd" = true)
  in b0 && b1 && b2 && b3 && br;;
...
let check_string_mapi f s r =
  ...
  let n2 = String.length r in
  if n1 = n2
  then string_andmapi (fun i' ih -> f i' (s.[i']) = ih)
         r
  else false;;
...
let check_string_reverse s s_op =
  let n1 = String.length s in
  let n2 = String.length s_op in
  if n1 = n2
  then string_andmapi (fun i' c -> (string_reverse_rec s).[i'] = s_op.[i'])
         s_op
  else false;;
